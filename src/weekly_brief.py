...
- 두 단락만 쓴다. 다른 제목·머리말·꼬리말을 붙이지 마라.
  ...
  2단락: 이번 주 시장 흐름이 내 포지션에 갖는 의미. pf_context 의 changes_this_week(이번 주 실제로 바뀐 것)를 우선 언급하고, 그 다음 standing_state 는 "구조적으로 이런 상태가 유지되고 있다" 정도로만 짧게 짚는다. 매주 반복되는 상시 상태를 길게 반복 설명하지 마라.
